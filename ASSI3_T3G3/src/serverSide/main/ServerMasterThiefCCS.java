...
public class ServerMasterThiefCCS
{
    ...
    public static void main (String[] args)
    {
        ...
        { GenericIO.writelnString ("CCS already bound exception: " + e.getMessage ());
            e.printStackTrace ();
            System.exit (1);
        }
        GenericIO.writelnString ("CCS object was registered!");

        /* wait for the end of operations */

        GenericIO.writelnString ("CCS is in operation!");
        try
        { while (!end)
            ...
            { GenericIO.writelnString ("CCS main thread was interrupted!");
            }
            }
        }
        catch (ClassNotFoundException e)
        { GenericIO.writelnString ("The data type ServerMasterThiefCCS was not found (blocking)!");
            e.printStackTrace ();
            System.exit (1);
        }
        ...
        { GenericIO.writelnString ("CCS deregistration exception: " + e.getMessage ());
            e.printStackTrace ();
            System.exit (1);
        }
        catch (NotBoundException e)
        ...
        GenericIO.writelnString ("CCS was deregistered!");

        try
        { shutdownDone = UnicastRemoteObject.unexportObject (collectionSite, true);
        }
        catch (NoSuchObjectException e)
        { GenericIO.writelnString ("CCS unexport exception: " + e.getMessage ());
            e.printStackTrace ();
            System.exit (1);
        }

        if (shutdownDone)
            GenericIO.writelnString ("Control and Collection Site was shutdown!");
    }
    ...
    public static void shutdown ()
    {
        ...
        { GenericIO.writelnString ("The data type ServerMasterThiefCCS was not found (waking up)!");
            e.printStackTrace ();
            System.exit (1);
        }
    }
