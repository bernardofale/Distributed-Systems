...
import serverSide.sharedRegions.OrdinaryThievesCS;
...
public class ServerOrdinaryThievesCS {
    ...
    {
        OrdinaryThievesCS CS;                                              // concentration site (service to be rendered)
        OrdinaryThievesCSInterface CSInter;                                // interface to the concentration site
        ...
        CS = new OrdinaryThievesCS (reposStub);                                      // service is instantiated
        CSInter = new OrdinaryThievesCSInterface(CS);                            // interface to the service is instantiated
        ...
        OrdinaryThievesCSProxy cliProxy;                                // service provider agent

        waitConnection = true;
        while (waitConnection)
        { try
        { sconi = scon.accept ();                                    // enter listening procedure
            cliProxy = new OrdinaryThievesCSProxy(sconi, CSInter);    // start a service provider agent to address
